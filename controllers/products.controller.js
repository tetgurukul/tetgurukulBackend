        ...
        */
        ...
        try {
           ...
           // Get file paths from req.files
            ...
            // const productImage = req.files && req.files.productImage ? await uploadToCloudinary(req.files.productImage[0]) : "";
            // const productSubImage = req.files && req.files.productSubImage ? await uploadToCloudinary(req.files.productSubImage[0]) : "";
            // const productCategoryImage = req.files && req.files.productCategoryImage ? await uploadToCloudinary(req.files.productCategoryImage[0]) : "";
...
//Below Api just get the Category of the products. Given my database structure, aggeraget queryi would be needing.
//Category Api
export const categoryOfProduct = async (req, res) => {
    try {
      const productCategoryData = await Product.aggregate([
        ...
        },
        {
          $project: {
            ...
            productCategoryImage: "$_id.productCategoryImage",
            productCategoryDescription: "$_id.productCategoryDescription",
            metaDescription: "$_id.metaDescription",
          }
        }
      ]);
  
      res.status(200).json({status:"success", data:productCategoryData});
    } catch (err) {
        res.status(500).json({status:"failed", message: error.message});
    }
  };
  ...
  //Below Api just get the SubCategory of the products. Given my database structure, aggeraget query would be needing.
//SubCategory Api


export const subCategoryOfProduct = async (req, res) => {
    try {

        const productSubCategoryData = await Product.aggregate ([

            {
                $group: {
                    _id: {
                        ...
                        productCategory:"$productCategory",
                        productCategoryImage: "$productCategoryImage",
                        productSubNameDescription: "$productSubNameDescription",
                        format: "$format",
                        metaTitle: "$metaTitle",
                        metaDescription:"$metaDescription",
                        slug: "$slug"
            ...
            },
            {
                $project: {
                    ...
                    _id: 0,
                    productId:"$_id.productId",
                    productSubName:"$_id.productSubName",
                    productSubImage:"$_id.productSubImage",
                    productCategory:"$_id.productCategory",
                    productCategoryImage:"$_id.productCategoryImage",
                    productSubNameDescription:"$_id.productSubNameDescription",
                    format:"$_id.format",
        ...
        res.status(200).json({status:"success", data:productSubCategoryData})
